...
const Chart: React.FC<IChartProps> = ({points}) => {
            ...
            points?.point ?
                {
                    ...
                    label: {
                        show: true,
                        formatter: `(${Number(points?.point[1].toFixed(2))}; ${Number(points?.point[0].toFixed(2))})`,
                        color: '#000',
    ...
    return (
        <div className={points == null ? 'hide' : ''}>
            ...
            {points?.point ? <Text size='l' align='left' className='text-under-chart'>Точка пересечения: ({Number(points?.point[1].toFixed(2))}; {Number(points?.point[0].toFixed(2))})</Text> : <Text>Точек пересечения нет</Text>}
        </div>
    )
}
